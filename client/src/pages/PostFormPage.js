...
function PostFormPage(props) {
  ...
  const handleContentChange = (event) => {
    const addy = {"address": props.values.stNum + " " + props.values.stName + ", " + props.values.city + ", " + props.values.state + ", " + props.values.zip};
    setAddress(addy);
    console.log("adress: ");
    console.log(addy);

    const rent = {"amount" : props.values.rent, "recieved": false, "dueDate": props.values.rentDueDate + "T01:02:03.955Z"};
    console.log(rent);
    setRent(rent);
  ...
  const handleSubmit = async (event) => {
    ...
    if(addressSuccess)
    {
      console.log("/api/houses/house/" + homeID + "/rent");
      try {
        let response = await fetch("/api/houses/house/" + homeID + "/rent", {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
    ...
    {
      try {
        let response = await fetch("/api/houses/house/" + homeID + "bills", {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="col-10 col-md-8 col-lg-7">
      ...
      <form onSubmit={handleSubmit}>
          ...
          <ul className = "list-group">
            ...
            {props.values.stNum + " " + props.values.stName + " " + props.values.city + " " + props.values.state + " " + props.values.zip}</li>
            <li className="list-group-item">Rent: {props.values.rent} due: {props.values.rentDueDate}</li>
            <li className="list-group-item">Mortgage: {props.values.mortgage} due: {props.values.mortDueDate}</li>
            <li className="list-group-item">Electric: {props.values.electric} due: {props.values.electricDueDate}</li>
            <li className="list-group-item">Gas: {props.values.gas} due: {props.values.gasDueDate}</li>
            <li className="list-group-item">Water: {props.values.water} due: {props.values.waterDueDate}</li>
          </ul>
